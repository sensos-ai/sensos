...
import { parseStableRelease, SemVer } from 'actions/autoship/lib/semver'
...
if (channel !== 'stable' || git('rev-parse', 'HEAD') !== sha)
...
for (const target of targets) {
  const name = `sensos-${target.asset}.tar.gz`
...
if (
  release &&
  (release.tag_name !== version.tagName || release.prerelease)
) {
  throw new Error(
...
const notes = (await Bun.file('CHANGELOG.md').text())
  .split(
    new RegExp(`## v${version.version.replaceAll('.', '\\.')}[^\\n]*\\n`)
  )[1]
  ?.split(/\n## v/)[0]
  ?.trim()
...
if (!release)
  release = await github('releases', {
    ...
    body: JSON.stringify({
      ...
      name: `Sensos CLI ${version.tagName}`,
      body: notes,
      draft: true,
      prerelease: false,
      make_latest: 'false',
...
for (const target of targets) {
  const name = `sensos-${target.asset}.intoto.jsonl`
  ...
  validateAttestation(bytes, `sensos-${target.asset}.tar.gz`)
  files.set(name, bytes)
}
const metadata = {
...
const advance =
  !state || (new SemVer(version.version).compare(state.version) ?? -1) >= 0
if (advance) {
...
// Give each SDK its own scoped release title and Changesets package tag.
// This also runs on retries where registry publication is already complete.
for (const name of libraries) {
  const tag = `@sensos-ai/${name}@${version.version}`
  await ensureTag(tag)
  const packageNotes = (
    await Bun.file(`packages/${name}/CHANGELOG.md`).text()
  )
    .split(`## ${version.version}\n`)[1]
    ?.split(/\n## /)[0]
    ?.trim()
  if (!packageNotes) throw new Error(`Missing changelog for ${tag}`)
