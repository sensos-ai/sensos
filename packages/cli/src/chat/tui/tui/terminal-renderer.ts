...
> = {
  ...
  connection: { color: colors.connection, border: '·' },
  error: { color: colors.error, border: '─' },
}

const connectionSpinnerFrames = [
  '⠋',
  '⠙',
  '⠹',
  '⠸',
...
export class TerminalRenderer {
  ...
  renderMessages(messages: UIMessage[], options?: TerminalSessionOptions) {
    this.#start(options)
    this.#sections = []
    this.#scrollOffset = 0
    const displayModes = {
    ...
    for (const message of messages) {
      if (message.role === 'user') {
        const text = message.parts
          ...
          .join('\n')
          .trim()
        if (text) this.#addUserSection(text)
    ...
    if (status === 'waking') {
      ...
      const render = () => {
        ...
        this.#upsertSection({
          ...
          kind: 'connection',
          title: 'Connection',
          content: `${spinner} Waking agent…`,
        })
        this.#paint()
      }
      render()
      this.#connectionSpinnerTimer = setInterval(render, 80)
      this.#connectionSpinnerTimer.unref?.()
      return
    }
      ...
      this.#onData = chunk => {
        ...
        switch (key.type) {
          ...
          case 'enter': {
            ...
            if (!prompt.startsWith('/')) this.#addUserSection(prompt)
            this.#inputText = ''
            this.#paint()
            this.#detachInput()
            resolve(prompt)
  ...
  #stopConnectionSpinner() {
    if (!this.#connectionSpinnerTimer) return
    clearInterval(this.#connectionSpinnerTimer)
    this.#connectionSpinnerTimer = undefined
  }
...
function isVisibleAssistantPart(
  ...
  switch (part.type) {
    ...
    case 'reasoning':
      return (
        displayModes.reasoning !== 'hidden' && part.text.trim().length > 0
      )
    default:
...
function renderSectionLines(section: ChatSection, width: number) {
  if (
    section.cache &&
    sectionMatchesCache(section, section.cache, width)
  ) {
