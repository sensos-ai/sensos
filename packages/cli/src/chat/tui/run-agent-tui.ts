...
export type RunAgentTUIOptions = {
  ...
  /** Durable chat/session identifier passed through to the transport. */
  chatId?: string

  /** Messages restored from the durable chat/session transcript. */
  initialMessages?: UIMessage[]

  /** Per-turn request options, evaluated once when a turn is submitted. */
  ...
  /**
   * A prompt run automatically as the first turn, before the terminal UI
   * goes interactive.
   */
  prompt?: string

  /** Slash commands available while entering a prompt. */
